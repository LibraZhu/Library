...
public class BottomBar extends FrameLayout
    ...
    private static final String STATE_CURRENT_SELECTED_TAB
            = "STATE_CURRENT_SELECTED_TAB";
    private static final String STATE_BADGE_STATES_BUNDLE
            = "STATE_BADGE_STATES_BUNDLE";
    private static final String TAG_BOTTOM_BAR_VIEW_INACTIVE
            = "BOTTOM_BAR_VIEW_INACTIVE";
    private static final String TAG_BOTTOM_BAR_VIEW_ACTIVE
            = "BOTTOM_BAR_VIEW_ACTIVE";
     ...
     * of your {@link Activity#onSaveInstanceState(Bundle)} to restore the
     * state.
     *
     * @param activity an Activity to attach to.
     * @param savedInstanceState a Bundle for restoring the state on
     ...
     * state.
     *
     * @param coordinatorLayout a CoordinatorLayout for the BottomBar to add
     * itself into
     * @param userContentView the view (usually a NestedScrollView) that has
    ...
    public static BottomBar attachShy(CoordinatorLayout coordinatorLayout, View userContentView, Bundle savedInstanceState) {
        ...
        bottomBar.toughChildHood(
                ViewCompat.getFitsSystemWindows(coordinatorLayout));
        bottomBar.onRestoreInstanceState(savedInstanceState);

        if (userContentView != null && coordinatorLayout.getContext()
                                                        .getResources()
                                                        .getBoolean(
                                                                R.bool.bb_bottom_bar_is_tablet_mode)) {
    ...
    /**
     * Set tabs and fragments for this BottomBar. When setting more than 3
     * items,
    ...
    public void setFragmentItems(android.app.FragmentManager fragmentManager,
                                 @IdRes
                                 int containerResource, BottomBarFragment... fragmentItems) {
                ...
                if (fragmentItem.getSupportFragment() == null &&
                    ...
                    throw new IllegalArgumentException(
                            "Conflict: cannot use android.support.v4.app.FragmentManager " +
                                    "to handle a android.app.Fragment object at position " +
                                    index +
     ...
     */
    public void setItemsFromMenu(
            @MenuRes int menuRes, OnMenuTabSelectedListener listener) {
        clearItems();
        mItems = MiscUtils.inflateMenuFromResource((Activity) getContext(),
                menuRes);
    ...
    public void selectTabAtPosition(int position, boolean animate) {
        if (mItems == null || mItems.length == 0) {
            throw new UnsupportedOperationException("Can't select tab at " +
                    "position " + position +
                    ". This BottomBar has no items set yet.");
        }
        ...
        unselectTab(mItemContainer.findViewWithTag(TAG_BOTTOM_BAR_VIEW_ACTIVE),
                animate);
    ...
    /**
     * Sets the default tab for this BottomBar that is shown until the user
     * changes
     * the selection.
     *
    ...
    public void setDefaultTabPosition(int defaultTabPosition) {
        if (mItems == null || mItems.length == 0) {
            throw new UnsupportedOperationException(
                    "Can't set default tab at " +
                            "position " + defaultTabPosition +
                            ". This BottomBar has no items set yet.");
        }
        else if (defaultTabPosition > mItems.length - 1 ||
                defaultTabPosition < 0) {
            throw new IndexOutOfBoundsException(
                    "Can't set default tab at position " +
                            defaultTabPosition +
                            ". This BottomBar has no items at that position.");
        }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        if (mFragmentManager != null && mFragmentContainer != 0 &&
                mItems != null && mItems instanceof BottomBarFragment[]) {
            BottomBarFragment bottomBarFragment
                    = (BottomBarFragment) mItems[mCurrentTabPosition];
            ...
            if (bottomBarFragment.getFragment() != null) {
                ...
                bottomBarFragment.getSupportFragment()
    ...
    public void mapColorForTab(int tabPosition, int color) {
        if (mItems == null || mItems.length == 0) {
            throw new UnsupportedOperationException(
                    "You have no BottomBar Tabs set yet. " +
     ...
     * using {@link #setActiveTabColor(int)}, as the default primary color
     * might
    ...
    public void useDarkTheme(boolean darkThemeEnabled) {
        if (!mIsDarkTheme && darkThemeEnabled && mItems != null &&
                mItems.length > 0) {
            darkThemeMagic();
    ...
    /**
     * Ignore the automatic Night Mode detection and use a light theme by
     * default,
     ...
     * three items.
     *
     * @param activeTabColor a hex color used for active tabs, such as
     * "#00FF000".
     */
    ...
    /**
     * Creates a new Badge (for example, an indicator for unread messages) for
     * a
     * Tab at
     ...
     *
     * @param typeFacePath path for the custom typeface in the assets
     * directory.
     */
    ...
    public void noNavBarGoodness() {
        if (mItems != null) {
            throw new UnsupportedOperationException(
                    "This BottomBar already has items! " +
     ...
     *
     * @param listener {@link OnSizeDeterminedListener} to get the size when
     * it's ready.
     */
    public void getBarSize(final OnSizeDeterminedListener listener) {
        ...
        if (sizeCandidate == 0) {
            mItemContainer.getViewTreeObserver()
                                  ...
                                  new ViewTreeObserver.OnGlobalLayoutListener() {
                                      ...
                                      public void onGlobalLayout() {
                                          ...
                                          if (Build.VERSION.SDK_INT >=
                                              ...
                                              obs.removeGlobalOnLayoutListener(
                                                      this);
                                          }
                                      }
                                  });
    ...
    /**
     * Get the actual BottomBar that has the tabs inside it for whatever what
     * you may want
     * to do with it.
     *
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mDarkBackgroundColor = ContextCompat.getColor(getContext(),
    ...
    private void initializeViews() {
        ...
        mUserContentContainer = (ViewGroup) rootView.findViewById(
                R.id.bb_user_content_container);
        mShadowView = rootView.findViewById(R.id.bb_bottom_bar_shadow);

        mOuterContainer = (ViewGroup) rootView.findViewById(
                R.id.bb_bottom_bar_outer_container);
        mItemContainer = (ViewGroup) rootView.findViewById(
                R.id.bb_bottom_bar_item_container);

        mBackgroundView = rootView.findViewById(
                R.id.bb_bottom_bar_background_view);
        mBackgroundOverlay = rootView.findViewById(
                R.id.bb_bottom_bar_background_overlay);
        ...
        if (mPendingUserContentView != null) {
            ViewGroup.LayoutParams params
                    = mPendingUserContentView.getLayoutParams();
    ...
    private void updateSelectedTab(int newPosition) {
        if (newPosition != mCurrentTabPosition) {
            ...
            if (mMenuListener != null && mItems instanceof BottomBarTab[]) {
                mMenuListener.onMenuItemSelected(
    ...
    private void handleBadgeVisibility(int oldPosition, int newPosition) {
        ...
        if (mBadgeMap.containsKey(oldPosition)) {
            BottomBarBadge oldBadge
                    = (BottomBarBadge) mOuterContainer.findViewWithTag(
                    mBadgeMap.get(oldPosition));
    ...
    @Override public boolean onLongClick(View v) {
        if ((mIsShiftingMode || mIsTabletMode) &&
                v.getTag().equals(TAG_BOTTOM_BAR_VIEW_INACTIVE)) {
            Toast.makeText(mContext,
                    mItems[findItemPosition(v)].getTitle(mContext),
    ...
    private void updateItems(final BottomBarItemBase[] bottomBarItems) {
        ...
        for (BottomBarItemBase bottomBarItemBase : bottomBarItems) {
            ...
            if (mBottomItemayoutResource != -1 && !mIsShiftingModeFlag) {
                layoutResource = mBottomItemayoutResource;
            }

            View bottomBarTab = View.inflate(mContext, layoutResource, null);
            ImageView icon = (ImageView) bottomBarTab.findViewById(
        ...
        if (!mIsTabletMode) {
            int proposedItemWidth = Math.min(MiscUtils.dpToPixel(mContext,
                    mScreenWidth / bottomBarItems.length), mMaxFixedItemWidth);

            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                    proposedItemWidth, LinearLayout.LayoutParams.WRAP_CONTENT);
        ...
        if (!mIsTabletMode) {
            ...
            mTabletRightBorder.setBackgroundColor(
                    ContextCompat.getColor(mContext,
                            R.color.bb_tabletRightBorderDark));
        }
    }
    ...
    private void onRestoreInstanceState(Bundle savedInstanceState) {
        if (savedInstanceState != null) {
            mCurrentTabPosition = savedInstanceState.getInt(
                    STATE_CURRENT_SELECTED_TAB, -1);
            mBadgeStateMap
                    = (HashMap<Integer, Boolean>) savedInstanceState.getSerializable(
                    STATE_BADGE_STATES_BUNDLE);
    ...
    private void handleBackgroundColorChange(View tab, int color) {
        MiscUtils.animateBGColorChange(tab, mBackgroundView, mBackgroundOverlay,
                color);
        mCurrentBackgroundColor = color;
    }
    ...
    private void updateCurrentFragment() {
        if (!mShouldUpdateFragmentInitially && mFragmentManager != null &&
                mFragmentContainer != 0 && mItems != null &&
                mItems instanceof BottomBarFragment[]) {
            BottomBarFragment newFragment
                    = ((BottomBarFragment) mItems[mCurrentTabPosition]);
            ...
            if (mFragmentManager instanceof android.support.v4.app.FragmentManager &&
                ...
                FragmentTransaction transaction
                        ...
                        .beginTransaction();
                for (BottomBarItemBase fragment : mItems) {
                    BottomBarFragment hideFragment
                            = ((BottomBarFragment) fragment);
                    transaction.hide(hideFragment.getSupportFragment());
                }
                if (!newFragment.getSupportFragment().isAdded()) {
                    transaction.add(mFragmentContainer,
                            newFragment.getSupportFragment());
                }
                transaction.show(newFragment.getSupportFragment())
                ...
                for (BottomBarItemBase fragment : mItems) {
                    BottomBarFragment hideFragment
                            = ((BottomBarFragment) fragment);
                    transaction.hide(hideFragment.getFragment());
                }
                if (!newFragment.getFragment().isAdded()) {
                    transaction.add(mFragmentContainer,
                            newFragment.getFragment());
                }
                transaction.show(newFragment.getFragment())
                           .commitAllowingStateLoss();
            }
        }
    ...
    private static void navBarMagic(Activity activity, final BottomBar bottomBar) {
        ...
        int softMenuIdentifier = res.getIdentifier("config_showNavigationBar",
                "bool", "android");
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT &&
            ...
            if (bottomBar.useTopOffset()) {
                int offset;
                int statusBarResource = res.getIdentifier("status_bar_height",
                        "dimen", "android");
                ...
                if (!bottomBar.useOnlyStatusbarOffset()) {
                    ...
                    if (activity.getTheme()
                                .resolveAttribute(android.R.attr.actionBarSize,
                                        tv, true)) {
                        offset += TypedValue.complexToDimensionPixelSize(
                                tv.data, res.getDisplayMetrics());
                    }
                    else {
                     ...
                     .addOnGlobalLayoutListener(
                             new ViewTreeObserver.OnGlobalLayoutListener() {
                                 ...
                                 public void onGlobalLayout() {
                                     ...
                                     if (bottomBar.isShy()) {
                                         int defaultOffset
                                                 = bottomBar.useExtraOffset()
                                                   ? navBarHeightCopy
                                                   : 0;
                                         bottomBar.setTranslationY(
                                                 defaultOffset);
                                         ((CoordinatorLayout.LayoutParams) bottomBar
                                     ...
                                     ViewTreeObserver obs
                                             = outerContainer.getViewTreeObserver();
    ...
    //add by libra
    private boolean mIsShiftingModeFlag;
    private int mBottomItemayoutResource = -1;//自定义底部栏item的布局


    /**
     * 设置之后bottomItemayoutResource失效， 设计遵从Material design规范
     */
    public void setIsShiftingModeFlag(boolean isShiftingModeFlag) {
        mIsShiftingModeFlag = isShiftingModeFlag;
    }


    /**
     * 自定义底部bar item的布局，调整高度or间距。(有违Material design规范(⊙﹏⊙))
     */
    public void setBottomItemayoutResource(int bottomItemayoutResource) {
        mBottomItemayoutResource = bottomItemayoutResource;
    }
}
