...
/**
 * Created by libra on 16/4/6 上午10:49.
 */
public class RxPermissions {
    ...
    /**
     * Map emitted items from the source observable into {@code true} if
     * permissions in parameters
     * are granted, or {@code false} if not.
     * <p>
     * If one or several permissions have never been requested, invoke the
     * related framework method
    ...
    public Observable.Transformer<Object, Boolean> ensure(final String... permissions) {
        return new Observable.Transformer<Object, Boolean>() {
            @Override public Observable<Boolean> call(Observable<Object> o) {
                return request(o, permissions)
                        // Transform Observable<Permission> to Observable<Boolean>
                        .buffer(permissions.length)
                        .flatMap(
                                new Func1<List<Permission>, Observable<Boolean>>() {
                                    @Override
    ...
    /**
     * Map emitted items from the source observable into {@link Permission}
     * objects for each
     * permissions in parameters.
     * <p>
     * If one or several permissions have never been requested, invoke the
    ...
    public Observable.Transformer<Object, Permission> ensureEach(final String... permissions) {
        return new Observable.Transformer<Object, Permission>() {
            @Override public Observable<Permission> call(Observable<Object> o) {
                return request(o, permissions);
            }
        };
    }
    ...
    /**
     * Request permissions immediately, <b>must be invoked during
     * initialization
     * phase
    ...
    private Observable<Permission> request(final Observable<?> trigger, final String... permissions) {
        ...
        return oneOf(trigger, pending(permissions)).flatMap(
                new Func1<Object, Observable<Permission>>() {
                    @Override public Observable<Permission> call(Object o) {
    ...
    private Observable<Permission> request_(final String... permissions) {
        ...
        if (!unrequestedPermissions.isEmpty()) {
            startShadowActivity(unrequestedPermissions.toArray(
    ...
    public Observable<Boolean> shouldShowRequestPermissionRationale(final Activity activity, final String... permissions) {
        ...
        return Observable.just(
                shouldShowRequestPermissionRationale_(activity, permissions));
    }
    ...
    void startShadowActivity(String[] permissions) {
        ...
        Intent intent = new Intent(mCtx, PermissionActivity.class);
    ...
    private boolean isGranted_(String permission) {
        return mCtx.checkSelfPermission(permission) ==
